...
def login():
    ...
    if (
        ...
        if user is not None:
            ...
            if role == "customer":
                cursor.execute(
                    "SELECT member_id FROM member WHERE user_name = %s", (username,)
                )
                result = cursor.fetchone()
                user_id = result["member_id"]
            elif role == "Manager":
                cursor.execute(
                    "SELECT manager_id FROM manager WHERE user_name = %s", (username,)
                )
                result = cursor.fetchone()
                user_id = result["manager_id"]
            elif role == "Instructor":
                cursor.execute(
                    "SELECT instructor_id FROM instructor WHERE user_name = %s",
                    (username,),
                )
                result = cursor.fetchone()
